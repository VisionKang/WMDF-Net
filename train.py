...
from pytorch_loss import Fusionloss1
    ...
    def train(self):
        ...
        while self.epoch <= self.nEpochs:
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                outputs = self.model(lr_img, pan_img)
                fusion_ap = torch.mean(outputs, dim=1, keepdim=True)
                ssim = ssim_loss(outputs, ms_img) # normalize=True
                mssim = mssim_loss(outputs, ms_img, normalize=True)
                train_loss = loss(pan_img, ms_img, outputs, fusion_ap)   # + 10*(1 - mssim)
                print("mssim = {:.5f},ssim = {:.5f}, train_loss = {:.5f}".format(mssim.item(), ssim.item(), train_loss.item()))
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for k, data in enumerate(self.eval_loader):
                ...
                # 网络输出
                eval_fused_images = self.model(img_lr, img_pan)
