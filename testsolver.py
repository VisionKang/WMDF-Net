...
from ert import Generator
from utils.utils import calculate_psnr, calculate_ssim, cc, sam, ergas, array2raster, denorm
...
class Testsolver(object):
    ...
    def test_all(self):
        ...
        for batch in self.test_dataloader:
            ...
            with torch.no_grad():
                test_fused_images = self.model2(img_lr, img_pan)  # 网络输出
            time1 = time.time()
            ...
            self.test_img_save(img_lr, 'test_ms_images', self.epoch2, count)
            self.test_img_save(img_pan, 'test_pan_images', self.epoch2, count)
            self.test_img_save(target,"real_images", self.epoch2, count)
    ...
    def test_img_save(self, x, name, epoch, count):
        ...
        test_dir = args.output_dir + '/QB/block_12/'
        real_dir = args.output_dir + '/QB/real/'
        lrms_dir = args.output_dir + '/QB/LRMS/'
        pan_dir = args.output_dir + '/QB/PAN/'
        ...
        if name == 'test_fused_images':
            ...
            array2raster(os.path.join(real_dir, '{}_mul.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].transpose(2, 0, 1)), 4)
        elif name == 'test_pan_images':
            array2raster(os.path.join(pan_dir, '{}_pan.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].reshape(x.shape[1], x.shape[2])), 1)
        else:
            array2raster(os.path.join(lrms_dir, '{}_lr.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].transpose(2, 0, 1)), 4)
